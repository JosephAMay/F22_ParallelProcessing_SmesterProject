...
int const NUMROW = 500;
int const  NUMCOL  = 500;
//function prototype
...
int main(int argc, char* argv[])
{
  ...
  //  cout << "DIAG DOMINANT IS =  " << diagDominant << endl;
   ...
   /*
    //input filename hardcoded her, can be set to command line argument as well
   infile = "500Matrix12";

  //create an array from read in file 
    createSysArray(infile, matrixArray, rhsArray, guessValue, answerArray );
...
void createRandArray(int numRow, int numCol, int arr[NUMROW][NUMCOL], int* sumArr)
{
    ...
    for (int x = 0; x < numRow; x++)
    {
        for (int y = x; y <= x; y++)
        {
            ...
            {
		/* //if multiplying diagonal elements to speedup diagonalization
		if(arr[x][y] == 0)
		{
			arr[x][y]=1;	
	    	} */
...
void arrayWriteToFile ( int arr[NUMROW][NUMCOL], int numrow, int numcol)
{
    ...
    myfile.open(filename + numAppendix);
...
int i,j;
int count = 0;
	/*for( i =NUMROW-1; i> NUMROW-3; i--)
	{

		for(j=NUMCOL-1; j>NUMCOL-3; j--)
		{
			cout << matrix[i][j] <<" ";
		}
		cout << " i= " << i << " j = " << j << endl;
